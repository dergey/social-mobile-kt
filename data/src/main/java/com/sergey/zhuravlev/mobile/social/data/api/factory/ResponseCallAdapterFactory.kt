package com.sergey.zhuravlev.mobile.social.data.api.factory

import com.sergey.zhuravlev.mobile.social.data.api.common.ErrorResponse
import com.sergey.zhuravlev.mobile.social.data.api.common.Response
...
class ResponseCallAdapterFactory : CallAdapter.Factory() {
  ...
  override fun get(
    ...
    val responseInstance =
      getRawType(responseType).newInstance() as Response<*, ErrorResponse.ErrorEntry>
    ...
    if (!ErrorResponse.ErrorEntry::class.java.isAssignableFrom(getRawType(errorType))) {
      return null
    }
    val errorBodyConverter: Converter<ResponseBody, ErrorResponse.ErrorEntry> =
  ...
  private fun fetchErrorType(responseType: Type): Type? {
    return try {
      ...
      if (ErrorResponse.ErrorEntry::class.java.isAssignableFrom(errorType)) {
        errorType
      } else {
        null
      }
...
class ResponseCallAdapter<S, E : ErrorResponse.ErrorEntry>(
...
class ResponseCall<S, E : ErrorResponse.ErrorEntry>(
